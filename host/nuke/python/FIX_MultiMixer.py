...
Version : 2.1
Changes : Add Copy Paste buttons
...
def MultiMixer():
    ...
    merge = nuke.nodes.Merge2(selected=1, operation='plus')
    ...
    for i, (n, read) in enumerate(valid_inputs):
        ...
        if i < len(valid_inputs) - 1:
            merge = nuke.nodes.Merge2(selected=1, operation='plus', inputs=(merge,))
    ...
    frange = nuke.nodes.FrameRange(selected=1, first_frame=first, last_frame=last, inputs=[merge])
    ...
    node.addKnob(nuke.Text_Knob('title', '', 'MultiMixer for ROLEX v2.1'))
...
new_node = FIX_MultiMixer.duplicate(n)
...
'''
    ...
    copy_script = '''
n = nuke.thisGroup()
reload(FIX_MultiMixer)
FIX_MultiMixer.copy_values(n)
'''
