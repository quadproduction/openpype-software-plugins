...
def alignNodes(direction):
    """
        USER SETTINGS
        when nodes are about to overlap as a result of an alignment,
        the nodes are placed next to each other instead.
        the multiplier variables define the amount of space that's
        kept between the nodes.
    ...
    # --------------------------------------
    # MULTIPLE NODES
    # if multiple nodes are selected, all the nodes will align to the node
    # that's the furthest away in the specified direction
    # --------------------------------------
    ...
    if len(selection) > 1:
        ...
        # check whether all selected nodes already share the same position
        # values to prevent overlapping
        # if so, do nothing
    ...
    # connected node (both input and output) in the specified direction
    # --------------------------------------
        ...
        # not every node has a hide input knob (read node for example),
        # so use a "try" in case it hasn't
        ...
        for i in connectedNodes:
            ...
            positions.append(
                [xPos, yPos, xPos - curNodexPos, yPos - curNodeyPos, i]
            )

        # sort the list based on the relative positions
        sortedList = sorted(positions, key=operator.itemgetter(index + 2))

        # remove nodes from list to make sure the first item is the node
        # closest to the curNode
        # use the operator module to switch dynamically between ">=" and "<="
        # the positiveDirection variable is used later to correctly calculate
        # to offset in case nodes are about to overlap
        ...
        if len(sortedList) != 0:
            ...
            # If they are about to overlap the node will be placed next to
            # the node it tried to snap to.
            for i in positions:
                # calculate the difference between the destination and the
                # position of the node it will align to
                difference = [
                    (abs(i[0][0] - destinationPosition[0])) * 1.5,
                    (abs(i[0][1] - destinationPosition[1])) * 1.5,
                ]

                # define the amount of units a node should offsetted to
                # not overlap
                offsetX = 0.75 * (
                ...
                # check in both directions whether the node is about to
                # overlap:
                if difference[0] < offsets[0] and difference[1] < offsets[1]:
                    ...
                    # find out whether the nodes are already very close to
                    # each other
                    ...
                    # stop looping through the list when a suitable node to
                    # align to is found
                    break
        else:
            dontmove = True
...
def getScreenSize(node):
    """
        To get the position of a node in the DAG you can use the xpos/ypos
        knobs. However, that position is heavely influenced by the size of
        the node. When horizontally aligned, a Dot node will have a different
        ypos than a blur node for example.
        To neuralize a nodes postionvalues you have to add the half the nodes
        screen dimensions to the positionvalues.
    """
