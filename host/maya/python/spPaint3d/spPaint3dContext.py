...
class intersectionPoint(object):
    ...
    def __init__(self, hitPoint, hitFace, hitTriangle, dagMesh):
        ...
        # MDagPath where the intersection occured (used to avoid having
        # to compute all normals on soon-to-be discarded intersections
        self.dagMeshTargetSurface = dagMesh

        # used to track the creation of an object at that
        # intersectionPoint (later used in the strokePointList class)
        self.timestamp = None

        # used to store the DAG path of the created geometry if it's
        # actually a valid intersection
        self.dagMeshSourceObject = None

        # used to store the DAG path of the created object
        self.generatedDAG = None

        # used to store the self.generatedDAG initial scale
        self.initialScale = [1, 1, 1]

    def getHitNormal(self, smooth=False):
        """
        return the normal (MVector) at the self.hitPoint, compute the normal
        differently according to the smooth boolean argument
        """
        if smooth:
            ...
            fnMesh.getClosestNormal(
                self.hitPoint.asMPoint(), normal, om.MSpace.kWorld, None
            )
            return normal
        else:
            ...
            return self.getCrossProduct(
                triVertsArray[0], triVertsArray[1], triVertsArray[2]
            )

    def convertUnit(self, currentUnit):
        """
        will convert the stored intersection internal coordinates into the
        current unit of the scene
        """
        if currentUnit != "cm":
            self.hitPoint.x = float(
                mc.convertUnit(
                    self.hitPoint.x, fromUnit="cm", toUnit=currentUnit
                )
            )
            self.hitPoint.y = float(
                mc.convertUnit(
                    self.hitPoint.y, fromUnit="cm", toUnit=currentUnit
                )
            )
            self.hitPoint.z = float(
                mc.convertUnit(
                    self.hitPoint.z, fromUnit="cm", toUnit=currentUnit
                )
            )
    ...
    def setInitialScale(self):
        ...
        """
        self.initialScale = mc.xform(
            self.generatedDAG, query=True, scale=True, r=True
        )
...
class intersectionList(object):
    ...
    def __init__(self, ipoint=None):
        ...
        if ipoint:
            self.addPoint(ipoint)
        if sp3d_log:
            print(
                "creating a new intersectionList (empty? %s)"
                % self.intersectionList
            )
    ...
    def addPoint(self, ipoint):
        ...
        if sp3d_log:
            print(
                """adding a new intersection to intersectionList
                (length of list: %i)"""
                % len(self.intersectionList)
            )
    ...
    def getClosest(self, sortorigin):
        """
        will parse the list to return the closest intersectionPoint to the
        sortorigin argument
        ...
        if length == 0:
            ...
            return self.intersectionList[0]
        else:
            ...
            for intersection in self.intersectionList:
                if not closestintersection:
                    ...
                    closestdistance = getDistanceBetween(
                        intersection.hitPoint, sortorigin
                    )
                else:
                    # compare distances
                    newdistance = getDistanceBetween(
                        intersection.hitPoint, sortorigin
                    )
    ...
    def printList(self):
        ...
        for obj in self.intersectionList:
            print(
                "object created: %s (using source: %s)"
                % (obj.generatedDAG, obj.dagMeshSourceObject)
            )
...
class paintContext(object):
    ...
    def __init__(self, uioptions, transformoptions, sourcelist, targetlist):
        ...
        if (mc.upAxis(q=True, axis=True)) == "y":
            ...
            mc.confirmDialog(
                ...
                message="""Not getting any proper info on what the current up
                vector is. Quitting...""",
            )
            sys.exit()
    ...
    def onPress(self):
        ...
        # initialise the intersection list that will contain all the created
        # objects within the same stroke
        self.strokeIntersectionList = intersectionList()

        # create the temporary group used through the stroke to store geometry
        # as they are created
        self.tempgroup = mc.group(empty=True, name=spPaint3dTempGroupID)

        pressPosition = mc.draggerContext(
            spPaint3dContextID, query=True, anchorPoint=True
        )

        worldPos, worldDir = getViewportClick(
            pressPosition[0], pressPosition[1]
        )

        intersected = targetSurfaceLoopIntersect(
            self.targetList, worldPos, worldDir
        )

        if intersected:
            # there was a usable intersection found

            # first converting the returned internal coordinates into the
            # current unit system
            ...
            if sp3d_log:
                print(
                    "intersection at X: %f | Y: %f | Z: %f"
                    % (
                        intersected.hitPoint.x,
                        intersected.hitPoint.y,
                        intersected.hitPoint.z,
                    )
                )
            ...
            if sp3d_dbg:
                logDebugInfo(
                    """finished creating object from the dag,
                    appending to intersection list"""
                )
            self.strokeIntersectionList.addPoint(intersected)

            if self.uiValues.jitter:
                # jittering the created object along the U & V parameters
                # from the UI
    ...
    def onDrag(self):
        ...
        dragPosition = mc.draggerContext(
            spPaint3dContextID, query=True, dragPoint=True
        )
        ...
        if intersected:
            # there was a usable intersection found, checking if it's coherent
            # with the paintFlux settings
            ...
            if len(self.strokeIntersectionList.intersectionList) == 0:
                ...
                if self.uiValues.paintFlux:
                    ...
                    if sp3d_log:
                        print(
                            """intersection at X: %f | Y: %f | Z: %f ||||
                            distance from previous: %f (x: %f | y: %f | z: %f)
                            (threshold: %f)(length of list: %i)"""
                            % (
                                intersected.hitPoint.x,
                                intersected.hitPoint.y,
                                intersected.hitPoint.z,
                                getDistanceBetween(
                                    self.strokeIntersectionList.intersectionList[  # noqa
                                        -1
                                    ].hitPoint,
                                    intersected.hitPoint,
                                ),
                    ...
                    if distanceToPrevious < correctedPaintDistance:
                        # distance with the new intersection is below
                        # threshold, forcing an end of proc
                        if sp3d_log:
                            print("breaking from onDrag since < threshold")
                    ...
                    if (
                        mc.timerX(
                            startTime=self.strokeIntersectionList.intersectionList[  # noqa
                                -1
                            ].timestamp
                        )
                        < self.uiValues.paintTimer
                    ):
                        # timer with the new intersection is below threshold,
                        # forcing an end of proc
                        intersected.isValid(False)
                    else:
            ...
            if intersected.isValid():
                ...
                # feed various data into the intersectionPoint object
                if self.uiValues.random:
                    intersected.updateDAGSourceObject(
                        self.sourceList.getRandom()
                    )
                else:
                    intersected.updateDAGSourceObject(
                        self.sourceList.getNext()
                    )
    ...
    def rampFX(self, objectList):
        ...
        if self.uiValues.rampFX:
            ...
            for object in objectList.intersectionList:
                # to force a float value so the division later on isn't
                # clamped to an integer division
                currentObj += 1.0

                if self.uiValues.rampFX != 1:
                    currentObjScaleY = (
                        currentObjScaleZ
                    ) = currentObjScaleX = scaleX[0] + scaleAmplitudeX * (
                        currentObj / nbObj
                    )
                    if not self.uiValues.transformScaleUniform:
                        currentObjScaleY = scaleY[0] + scaleAmplitudeY * (
                            currentObj / nbObj
                        )
                        currentObjScaleZ = scaleZ[0] + scaleAmplitudeZ * (
                            currentObj / nbObj
                        )
                    mc.scale(
                        currentObjScaleX * object.initialScale[0],
                        currentObjScaleY * object.initialScale[1],
                        currentObjScaleZ * object.initialScale[2],
                        object.generatedDAG,
                        relative=False,
                    )
                    if sp3d_ramp:
                        print(
                            """rampFX (Scale) obj# %i out of %i (percent: %f)
                            (name: %s) computed scale X %f / Y %f / Z %f
                            (max amplitude X %f)"""
                            % (
                                currentObj,
                                nbObj,
                ...
                if self.uiValues.rampFX != 2:
                    currentObjRotateX = rotateX[0] + rotateAmplitudeX * (
                        currentObj / nbObj
                    )
                    currentObjRotateY = rotateY[0] + rotateAmplitudeY * (
                        currentObj / nbObj
                    )
                    currentObjRotateZ = rotateZ[0] + rotateAmplitudeZ * (
                        currentObj / nbObj
                    )
                    mc.rotate(
                        currentObjRotateX,
    ...
    def onRelease(self):
        ...
        if self.uiValues.hierarchy:
            ...
            if self.uiValues.group == 0.0:
                ...
                for obj in self.strokeIntersectionList.intersectionList:
                    shapeParent = mc.listRelatives(
                        obj.dagMeshSourceObject, parent=True
                    )
        ...
        if mc.objExists(self.tempgroup):
            ...
            if not mc.listRelatives(self.tempgroup, children=True):
                ...
                if sp3d_log:
                    print(
                        "tempGroup (%s) is empty, removing." % self.tempgroup
                    )
                mc.delete(self.tempgroup)

    def createObject(self, intersection):
        """
        will create the object at the intersection object gathered data,
        pending all ui and transform options will update the stored data
        to store the created object DAG path and return the newly created
        object DAG Path back
        """
        newObjectDAG = None
        if self.uiValues.instance:
            if sp3d_dbg:
                logDebugInfo("creating instance")
            # fetching the transform for that shape (instance dont create
            # object of child objects if it's the shape that gets instanced)
            tempDAG = mc.listRelatives(
                intersection.dagMeshSourceObject, parent=True
            )
            newObjectDAG = mc.instance(tempDAG[0])
        else:
            # fetching the parent transform to prevent some issue while
            # duplicating with preserve input connections and stuff
            # docked onto the transform and not the shape
            if sp3d_dbg:
                logDebugInfo("duplicating object")
            tempDAG = mc.listRelatives(
                intersection.dagMeshSourceObject, parent=True
            )
            newObjectDAG = mc.duplicate(
                tempDAG[0], ic=self.uiValues.preserveConn
            )
        if sp3d_dbg:
        ...
        if self.uiValues.align:
            ...
            rx, ry, rz = getEulerRotationQuaternion(
                self.worldUp,
                intersection.getHitNormal(self.uiValues.smoothNormal),
            )
        ...
        if self.uiValues.transformScale and not self.uiValues.rampFX:
            # scale transform
            randscale = self.transform.getRandomScale(
                self.uiValues.transformScaleUniform
            )
            mc.scale(
                randscale[0],
                randscale[1],
        ...
        if self.uiValues.hierarchy:
            ...
            if mc.objExists(self.tempgroup):
                grouped = mc.parent(
                    newObjectDAG[0], self.tempgroup, relative=True
                )
            else:
    ...
    def runtimeUpdate(
        self, uioptions, transformoptions, sourcelist, targetlist
    ):
        """
        entry method used from GUI to pass changes in the UI options through
...
class modifierManager(object):
    ...
    modifierMask = {"shift": 1, "ctrl": 4, "alt": 8}
    ...
    def resetCtrl(self):
        """
        reset ctrl back to default settings, usually after the key
    ...
    def getState(self):
        ...
        if (modifiers & 1) > 0:
            if self.shiftReleased:
                # shift was released in the previous iteration,
                # this is a keypress
                self.shiftReleased = False
                shift = True
        else:
            # shift is not currently pressed, reseting the tracking
            # self variable to released state
            self.shiftReleased = True

        # CTRL EVENT
        if (modifiers & 4) > 0:
            if self.ctrlReleased:
                # ctrl was released in the previous iteration,
                # this is a keypress
                self.ctrlReleased = False
                ctrl = True
        else:
            # ctrl is not currently pressed, reseting the tracking
            # self variable to released state
            self.ctrlReleased = True
        ...
        if (modifiers & 8) > 0:
            if self.altReleased:
                # alt was released in the previous iteration,
                # this is a keypress
                self.altReleased = False
                alt = True
        else:
            # alt is not currently pressed, reseting the tracking
            # self variable to released state
            self.altReleased = True
...
class placeCursor(object):
    ...
    def setCursorDAG(self, sourcedag, cursordag, deleteprevious=False):
        ...
        if deleteprevious:
            if mc.objExists(self.cursorDAG):
                # deleting the previous cursor object and its parent group
                # if it's empty
                parentgroup = mc.listRelatives(self.cursorDAG, parent=True)
                mc.delete(self.cursorDAG)
                if not mc.listRelatives(parentgroup):
                    mc.delete(parentgroup)
        self.cursorDAG = cursordag
        self.initialScale = mc.xform(
            self.cursorDAG, query=True, scale=True, r=True
        )
    ...
    def rotateCursor(self, increment):
        ...
        if self.rotationIncrement:
            self.rotationIncrement = [
                (self.rotationIncrement[0] + increment[0]),
                (self.rotationIncrement[1] + increment[1]),
                (self.rotationIncrement[2] + increment[2]),
            ]
        else:
            self.rotationIncrement = increment
    ...
    def align(self, rx=None, ry=None, rz=None):
        ...
        if (rx is not None) and (ry is not None) and (rz is not None):
            ...
            # realigning cursor with previously stored rotate tuple
            # if it exists
            if self.cursorAlign:
...
class placeContext(object):
    ...
    def fetchCursorObject(self):
        """
        gather necessary data and generate a new cursor Object and return
        the string to the source DAG and the cursor DAG
        """
        # fetch a new cursorObject
        if self.uiValues.random:
            sourceDAG = self.sourceList.getRandom()
        else:
            sourceDAG = self.sourceList.getNext()

        # checking if the sourceDAG is a transform, if not attempting to find
        # the parent transform instancing multiple object doesnt work
        # if using a shape duplicating input connections on the parent
        # transform is not carried over if using the shape
        ...
        if self.uiValues.instance:
            ...
            newObjectDAG = mc.duplicate(
                sourceDAG, ic=self.uiValues.preserveConn
            )
    ...
    def fetchCursorTransform(self):
        """
        compute transform data for cursor object and return the tuple
        ...
        if self.uiValues.transformScale:
            tempCursorScale = self.transform.getRandomScale(
                self.uiValues.transformScaleUniform
            )
            cursorScale = [
                tempCursorScale[0] * self.cursor.initialScale[0],
                tempCursorScale[1] * self.cursor.initialScale[1],
                tempCursorScale[2] * self.cursor.initialScale[2],
            ]
        else:
            # transform scale off
            cursorScale = (1, 1, 1)
    ...
    def ctrlEvent(self):
        ...
        self.cursor.setCursorDAG(
            newSourceDAG, newCursorDAG, True
        )  # flagging to delete previous cursor
        if self.uiValues.align:
            self.cursor.align()

        # defaulting without parameters to position the new cursor to
        # the previously stored position
        self.cursor.move(
            None, self.cursor.rotationIncrement
        )
        self.cursor.transform(
            self.uiValues.transformRotate, self.uiValues.transformScale
        )
        if self.uiValues.upOffset != 0:
            offsetArray = [
                self.uiValues.upOffset * self.worldUp.x,
    ...
    def shiftEvent(self):
        ...
        """
        rotateArray = [
            self.worldUp.x * self.uiValues.placeRotate,
            self.worldUp.y * self.uiValues.placeRotate,
            self.worldUp.z * self.uiValues.placeRotate,
        ]
        self.cursor.rotateCursor(rotateArray)
        if sp3d_log:
            print(self.cursor.rotationIncrement)
    ...
    def onPress(self):
        ...
        # initializing / reseting the rotation increment
        # if we are re-entering place
        ...
        if intersected:
            ...
            if self.uiValues.align:
                rx, ry, rz = getEulerRotationQuaternion(
                    self.worldUp,
                    intersected.getHitNormal(self.uiValues.smoothNormal),
                )
    ...
    def onDrag(self):
        ...
        if intersected:
            ...
            if self.uiValues.align:
                ...
                self.cursor.align(rx, ry, rz)
            self.cursor.move(
                intersected.hitPoint, self.cursor.rotationIncrement
            )
            ...
            if self.uiValues.transformRotate or self.uiValues.transformScale:
                ...
                if self.uiValues.continuousTransform:
                    ...
                    self.cursor.setCursorTransform(cursorRotate, cursorScale)
                    self.cursor.transform(
                        self.uiValues.transformRotate,
                        self.uiValues.transformScale,
                    )
    ...
    def onHold(self):
        ...
        if sp3d_log:
            ...
            if ctrl:
                message += "ctrl pressed... "
            if shift:
                message += "shift pressed... "
            if alt:
                message += "alt pressed... "
            print(message)
    ...
    def onRelease(self):
        ...
        if self.uiValues.hierarchy:
            ...
            if self.uiValues.group == 0.0:
                ...
                # source group sorting
                shapeParent = mc.listRelatives(
                    self.cursor.sourceDAG, parent=True
                )
                groupName = "spPaint3dOutput_" + shapeParent[0]
                if not mc.objExists(groupName):
                    # group doesnt exists, creating
...
def moveTo(dag, pos, rot=None):
    ...
    attemps to compensate for unfrozen transform by reading
    the scalepivot of the object
    """
    ...
    mc.xform(
        dag,
        t=(
            (transform[0] - scalePivot[0]) + pos.x,
            (transform[1] - scalePivot[1]) + pos.y,
            (transform[2] - scalePivot[2]) + pos.z,
        ),
    )
    if rot:
        if sp3d_log:
...
def getPosition(dag):
    """
    retrieve the world position of the dag parameter object and return a
    ...
    return point(
        (transform[0] - scalePivot[0]),
        (transform[1] - scalePivot[1]),
        (transform[2] - scalePivot[2]),
    )
...
def getViewportClick(screenX, screenY):
    """
    return world position and direction of the viewport clicked point
...
def targetSurfaceLoopIntersect(targetList, clickPos, clickDir):
    """
    loop through all the object in targetList and intersect them with click
    (world pos, direction). creates an intersectionPoint object for each
    intersection. sort the list of intersection and return the closest
    intersectionPoint object from the click world position,
    ...
    for obj, data in targetList.obj.items():
        ...
        intersected = intersectTargetSurface(
            data[0], clickPos, clickDir, farclip
        )
        if intersected:
...
def intersectTargetSurface(targetdag, clickPos, clickDir, farclip=1.0):
    """
    intersect a single object from the click world pos and direction.
    optional farclip distance
    ...
    if targetDAGPath:
        ...
        if hit:
            ...
            if sp3d_MFn:
                print(
                    "Face Hit: %i || Tri Hit: %i"
                    % (hitFace.getInt(hitFaceptr), hitTri.getInt(hitTriptr))
                )
            return intersectionPoint(
                point(currentHitFP.x, currentHitFP.y, currentHitFP.z),
                hitFace.getInt(hitFaceptr),
                hitTri.getInt(hitTriptr),
                targetDAGPath,
            )
...
def getCorrectedDistance(distance, unit):
    """
    return the corrected distance using proper unit to convert
    back to centimeters
    """
    if unit == "cm":
        return distance
    else:
        return distance / (sp3dUnit[unit])
