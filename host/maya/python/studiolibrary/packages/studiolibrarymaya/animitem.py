...
class AnimCreateWidget(basecreatewidget.BaseCreateWidget):
    ...
    def showByFrameDialog(self):
        ...
        """
        text = (
            'To help speed up the playblast you can set the "by frame" '
...
class AnimPreviewWidget(basepreviewwidget.BasePreviewWidget):
    def __init__(self, *args, **kwargs):
        ...
        self.connect(
            self.ui.currentTime, QtCore.SIGNAL("stateChanged(int)"), self.saveSettings
        )
        self.connect(
            self.ui.helpCheckBox, QtCore.SIGNAL("stateChanged(int)"), self.showHelpImage
        )
        self.connect(
            self.ui.connectCheckBox,
    ...
    def accept(self):
        ...
        self.item().loadFromSettings(sourceStart=sourceStart, sourceEnd=sourceEnd)
