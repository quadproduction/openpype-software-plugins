...
VALID_CONNECTIONS = ["animCurve", "animBlend", "pairBlend", "character"]
...
class Attribute(object):
    ...
    def insertStaticKeyframe(self, value, time):
        ...
        try:
            ...
            nextFrame = maya.cmds.findKeyframe(
                self.fullname(), time=(endTime, endTime), which="next"
            )
            maya.cmds.keyTangent(
                self.fullname(), time=(nextFrame, nextFrame), itt="flat"
            )
        except TypeError as e:
    ...
    def isSettable(self, validConnections=None):
        ...
        if not maya.cmds.listAttr(
            self.fullname(), unlocked=True, keyable=True, multi=True, scalar=True
        ):
            return False
