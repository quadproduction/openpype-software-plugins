...
class Pose(mutils.SelectionSet):
        ...
        for attr in attrs:
            if attr.isValid():
                if attr.value() is None:
                    logger.warning(
                        "Cannot save the attribute %s with value None."
                        % attr.fullname()
                    )
                else:
    ...
    def setMirrorAxis(self, name, mirrorAxis):
        ...
        if name in self.objects():
            ...
            logger.debug(
                "Object does not exist in pose. Cannot set mirror axis for %s" % name
            )
    ...
    def setMirrorTable(self, mirrorTable):
        ...
        mirrorTable.matchObjects(
            objects=list(self.objects().keys()), callback=self.updateMirrorAxis
        )
    ...
    def cacheNode(
        ...
        if self.mirrorTable():
            ...
            if not mirrorObject:
                ...
                logger.debug(
                    "Cannot find mirror object in pose for %s" % srcNode.name()
                )
            ...
            if mirrorObject and not maya.cmds.objExists(mirrorObject):
                logger.debug(
                    "Mirror object does not exist in the scene %s" % mirrorObject
                )
        ...
        for attr in self.attrs(srcNode.name()):
            ...
            srcMirrorValue = self.attrMirrorValue(
                mirrorObject, attr, mirrorAxis=mirrorAxis
            )

            srcAttribute = mutils.Attribute(
                dstNode.name(), attr, value=value, type=type_
            )
            dstAttribute.update()
