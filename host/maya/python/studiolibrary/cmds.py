...
class PathError(IOError):
    ...
    def __init__(self, msg):
        ...
        :type: str or unicode
        """
        msg = str(msg).encode("unicode_escape")
    ...
    def __unicode__(self):
        ...
        :rtype: unicode
        """
        msg = str(self._msg).decode("unicode_escape")
        return msg
...
def itemClasses():
    ...
    """
    return list(_itemClasses.values())
...
def findItems(path, direction=Direction.Down, depth=3, **kwargs):
    ...
    :rtype: collections.Iterable[studiolibrary.LibraryItem]
    """
    paths = findPaths(path, match=isValidItemPath, direction=direction, depth=depth)
...
def isMac():
    ...
    """
    return (
        system().startswith("os")
        or system().startswith("mac")
...
def formatPath(formatString, path="", **kwargs):
    ...
    return str(formatString).format(**kwargs)
...
def movePath(src, dst):
    ...
    if not os.path.exists(src):
        raise MovePathError("No such file or directory: {0}".format(src))

    if os.path.isdir(src):
        dst = "{0}/{1}{2}".format(dst, name, extension)
...
def movePaths(srcPaths, dst):
    ...
    for src in srcPaths or []:
        ...
        logger.info("Moving Content: {0} => {1}".format(src, dst_))
...
def renamePath(src, dst, extension=None, force=False):
    ...
    logger.debug("Renaming: {0} => {1}".format(src, dst))

    if src == dst and not force:
        msg = "The source path and destination path are the same: {0}"
        raise RenamePathError(msg.format(src))

    if os.path.exists(dst) and not force:
        msg = 'Cannot save over an existing path: "{0}"'
        raise RenamePathError(msg.format(dst))

    if not os.path.exists(dirname):
        msg = 'The system cannot find the specified path: "{0}".'
    ...
    logger.debug("Renamed: {0} => {1}".format(src, dst))
...
def readJson(path):
    ...
    logger.debug("Reading json file: {0}".format(path))
...
def generateUniquePath(path, attempts=1000):
    ...
    while os.path.exists(path):
        ...
        path = path_.format(
            name=name, number=attempt, dirname=dirname, extension=extension
        )
...
def findPaths(path, match=None, direction=Direction.Down, depth=3, **kwargs):
    """
...
def showInFolder(path):
    ...
    if SHOW_IN_FOLDER_CMD:
        ...
        args = [None, "open", "explorer.exe", '/n,/select, "{path}"', None, 1]
...
def testUpdate():
    ...
    """
    testData1 = {
        "../../images/beach.jpg": {"Custom Order": "00001"},
        "../../images/sky.jpg": {
            "Custom Order": "00019",
...
def testSplitPath():
    ...
    expected = ("P:/production/rigs/character", "mario", ".ma")
