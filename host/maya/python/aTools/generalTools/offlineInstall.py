...
def aToolsOfflineInstall(offlineFilePath):
    ...
    zfobj.close()
    if os.path.isfile(tmpZipFile):     os.remove(tmpZipFile)
    from host.maya.python.aTools import setup; reload(setup); setup.install([offlineFilePath, False])
    cmds.evalDeferred(\"from host.maya.python.aTools.animTools.animBar import animBarUI; reload(animBarUI); animBarUI.show(\'refresh\')\")
