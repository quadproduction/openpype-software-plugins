...
from host.maya.python.aTools.commonMods      import commandsMod
from host.maya.python.aTools.commonMods      import aToolsMod
from host.maya.python.aTools                 import setup
from host.maya.python.aTools.generalTools    import hotkeys;                     importlib.reload(hotkeys)
from host.maya.python.aTools.generalTools    import tumbleOnObjects;             importlib.reload(tumbleOnObjects)
from host.maya.python.aTools.animTools       import animationCrashRecovery;      importlib.reload(animationCrashRecovery)
from host.maya.python.aTools.animTools       import framePlaybackRange;          importlib.reload(framePlaybackRange)
from host.maya.python.aTools.animTools       import jumpToSelectedKey;           importlib.reload(jumpToSelectedKey)
...
class GeneralTools_Gui(uiMod.BaseSubUI):
    ...
    def uninstall(self, *args):
        ...
        if confirm == 'Yes':
            ...
            from host.maya.python.aTools.animTools.animBar import animBarUI; importlib.reload(animBarUI)
...
class Update(object):
    ...
    def hasUpdate(self):
        ...
        if utilMod.hasInternet(UPDATE_URL):
            try:    response = urllib.request.urlopen(UPDATE_URL, timeout=60)
            except: pass
        ...
        try:
            ...
            if str.encode(VERSION) not in latestVersion:
...
def shelfButton(*args):
    ...
    command         = "from host.maya.python.aTools.animTools.animBar import animBarUI; animBarUI.show('toggle')"
...
def installAnimBot(*args):
    installFileFolder = os.path.dirname(os.path.dirname(__file__))
    installFilePath = os.path.join(installFileFolder, "animBot Drag'n Drop Install.mel")
