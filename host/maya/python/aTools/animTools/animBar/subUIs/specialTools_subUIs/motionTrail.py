...
from host.maya.python.aTools.generalTools.aToolsGlobals import aToolsGlobals as G
from host.maya.python.aTools.commonMods import uiMod
from host.maya.python.aTools.commonMods import utilMod
from host.maya.python.aTools.commonMods import animMod
from host.maya.python.aTools.commonMods import aToolsMod
...
class MotionTrail(object):
    ...
    def populateMenu(self, menu, *args):
        ...
        if cmds.objExists(self.mainGroupNode):
            for loopMotionTrail in list(self.nodeInfo.keys()):
    ...
    def removeMotionTrail(self, motionTrail):
        ...
        if motionTrail in self.nodeInfo:
        ...
        if len(list(self.nodeInfo.keys())) == 0: self.clear()
    ...
    def callbackCurveEdited(self, *args):
        ...
        curveMsg                = args[0]
        self.lastCurvesEdited   = [OpenMaya.MFnDependencyNode(curveMsg[n]).name() for n in range(curveMsg.length())]
    ...
    def getTotalDirty(self):
        ...
        for loopMotionTrail in list(self.nodeInfo.keys()):
            ...
            for loopFrame in self.sortedRange:
                ...
                if loopFrame not in self.nodeInfo[loopMotionTrail]:
                    totalDirty += 1
                    continue
    ...
    def getNextBit(self):
        ...
        for loopFrame in self.sortedRange:
            for loopMotionTrail in list(self.nodeInfo.keys()):
                ...
                if loopFrame in self.nodeInfo[loopMotionTrail]:
    ...
    def makeDirty(self, motionTrail):
        ...
        for loopFrame in self.sortedRange:
            ...
            if loopFrame not in self.nodeInfo[motionTrail]: self.nodeInfo[motionTrail][loopFrame] = {}
    ...
    def createMotionTrail(self, ctrls):
        ...
        for loopCtrl in ctrls:
            ...
            self.nodeInfo[ctrlName]     = {"ctrlNode":loopCtrl, "offsetNode":offsetNode, "groupNode":groupNode, "motionTrailTransform":motionTrailTransform, "motionTrailAttr":motionTrailAttr, "motionTrailNode":motionTrailNode, "lineColor":next(self.lineColorCycle)}
            ...
            if ctrlName in self.offsetCtrlsPosition:
    ...
    def saveOffset(self, motionTrails=None):
        ...
        motionTrails = motionTrails or list(self.nodeInfo.keys())
    ...
    def updateKeysTimes(self):
        ...
        self.keysTimes       = self.keysTimesDict[list(self.keysTimesDict.keys())[0]] if len(self.keysTimesDict) > 0 else []
    ...
    def refreshMotionTrail(self, *args):
        ...
        for loopMotionTrail in list(self.nodeInfo.keys()): self.makeDirty(loopMotionTrail)
    ...
    def getCameraPoint(self, camera, frame, currFrame, pointMatrix):
        ...
        if currFrame not in self.cameraInfo[camera]: self.addFrameCameraMatrices(currFrame)
    ...
    def getPointsArray(self, motionTrail, currFrame, camera=None):
        ...
        for loopFrame in rangeList:
            ...
            if loopFrame in self.nodeInfo[motionTrail]:
                if "pointMatrix" in self.nodeInfo[motionTrail][loopFrame]:
    ...
    def getIndexMap(self, motionTrail, keysTimes, currFrame, pointArray):
        ...
        for loopIndex in range(len(pointArray)):
    ...
    def drawViewports(self, *args):
        ...
        for loopViewport in utilMod.getAllViewports():
            ...
            for loopMotionTrail in list(self.nodeInfo.keys()):
                ...
                camera          = viewportCamera if viewportCamera in list(self.cameraInfo.keys()) and self.cameraRelativeMode else None
                points          = self.getPointsArray(loopMotionTrail, currFrame, camera)
                pointArray      = points["camera"] if camera else points["world"]
                loopKeysTimes   = [] if loopMotionTrail not in self.keysTimesDict else self.keysTimesDict[loopMotionTrail]
